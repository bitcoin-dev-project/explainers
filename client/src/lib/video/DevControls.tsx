...
export function DevControls({ player }: { player: UseVideoPlayerReturn }) {
  const { currentScene, totalScenes, prev, next, goToScene } = player;

  // Hide when recording (?record in URL)
  if (new URLSearchParams(window.location.search).has('record')) return null;
